...
router.get("/", async (req, res) => {
  session = req.session;
  if (session.userid) {
    let query = `select user_role,user_id, uname from adbatch1btr_user where username='${session.userid}'`;
    const sol = await connection.execute(query);
    const user_role = sol["rows"][0][0];
    const user_id = sol["rows"][0][1];
    const uname = sol["rows"][0][2];
    if (user_role == "USER") {
      let query = `select  from_city,to_city,status, date_of_journey, reason from adbatch1btr_tripdetails where user_id=${user_id}`;
      let trips = await connection.execute(query);
      trips = trips.rows;
      res.render("user_dashboard", { trips, message: "" });
    } else if (user_role != "ADMIN") {
      let query = `select t.trip_id, t.from_city, t.to_city, t.from_country, t.to_country, t.accomodation, t.reason, t.date_of_journey, t.amount, t.currency, t.status, u.uname, u.date_of_joining, u.nationality, u.date_of_birth, u.passport_number from adbatch1btr_tripdetails t natural join adbatch1btr_user u where t.pending_with='${user_role}'`;
      let trips = await connection.execute(query);
      trips = trips.rows;
      res.render("super_dashboard", { trips, message: "" });
    } else if (user_role == "ADMIN") {
      let query = `select t.trip_id, t.from_city, t.to_city, t.from_country, t.to_country, t.accomodation, t.reason, t.date_of_journey, t.amount, t.currency, t.status, u.uname, u.date_of_joining, u.nationality, u.date_of_birth, u.passport_number from adbatch1btr_tripdetails t natural join adbatch1btr_user u where status = 'In Process'`;
      let trips = await connection.execute(query);
      trips = trips.rows;
...
router.get("/create", async (req, res) => {
  if (req.session.userid) {
    const query = `select uname, date_of_joining, nationality, date_of_birth, passport_number from adbatch1btr_user where username = '${req.session.userid}'`;
...
router.get("/approve/:trip_id/:status", async (req, res) => {
  if (!req.session.userid) res.redirect("/account/login");
    ...
    let query = `Select user_role from adbatch1btr_user where username ='${session.userid}'`;
    const sol = await connection.execute(query);
    const user_role = sol["rows"][0][0];
    ...
    if (user_role != "USER") {
      ...
      query = `update adbatch1btr_tripdetails set status = '${status}', pending_with = '${pending_with}' where trip_id = '${trip_id}'`;
